    ...
    : public Loggable
{
  public:
    ...
    virtual ValidatedVectorMultivariateFunctionModel implicit(const ValidatedVectorMultivariateFunction& f, const ExactBoxType& A, const ExactBoxType& X) const = 0;
    //! \brief Solve \f$f(a,x)=0\f$ for a in \a A, looking for a solution with x in \a X.
    virtual ValidatedScalarMultivariateFunctionModel implicit(const ValidatedScalarMultivariateFunction& f, const ExactBoxType& A, const ExactIntervalType& X) const = 0;

    //! \brief Solve \f$f(a,x)=0\f$ for x in \a X, and continue to a function \f$h\f$ solving \f$f(a,h(a))=0\f$ over \f$A\f$.
    virtual ValidatedVectorMultivariateFunctionModel continuation(const ValidatedVectorMultivariateFunction& f, const Vector<ApproximateNumericType>& a, const ExactBoxType& X,  const ExactBoxType& A) const = 0;
