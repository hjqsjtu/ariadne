...
struct RealBase : RealInterface {
    ValidatedReal _compute(Accuracy acc) const;
...
struct RealWrapperBase : RealBase {
    FloatDPBounds _compute_get(Effort eff, DoublePrecision pr) const { return this->_compute(eff).get(pr); }
    FloatMPBounds _compute_get(Effort eff, MultiplePrecision pr) const { return this->_compute(eff).get(pr); }
};

MultiplePrecision precision(Nat pr) { return MultiplePrecision(pr); }

struct RealExpressionBase : RealWrapperBase {
    virtual DyadicBounds _compute_get(Effort eff) const { return DyadicBounds(this->_compute_get(eff, precision(eff.work()+2))); }
...
template<class O, class A> struct RealWrapper<O,A> : virtual RealBase, Symbolic<O,A>, FloatDPBounds {
    RealWrapper(O o, A a) : Symbolic<O,A>(o,a)
        , FloatDPBounds(this->_op(this->_arg.get(dp))) { }
    virtual DyadicBounds _compute_get(Effort eff) const { return DyadicBounds(this->_compute_get(eff, Ariadne::precision(eff.work()+2))); }
...
template<class O, class A1, class A2> struct RealWrapper<O,A1,A2> : virtual RealBase, Symbolic<O,A1,A2>, FloatDPBounds {
...
template<class A, class N> struct RealWrapper<Pow,A,N> : virtual RealBase, Symbolic<Pow,A,N>, FloatDPBounds {
...
template<class X> struct RealConstant : RealBase, FloatDPBounds {
    X _c;
  public:
    RealConstant(X const& x) : FloatDPBounds(x,dp), _c(x) { }
    virtual DyadicBounds _compute_get(Effort eff) const { return DyadicBounds(this->_compute_get(eff, MP(eff.work()+2))); }
...
template<> struct RealConstant<FloatDPBounds> : RealBase, FloatDPBounds {
...
template<> struct RealConstant<EffectiveNumber> : RealBase, FloatDPBounds {
...
DyadicBounds WhenRealExpression::_compute_get(Effort eff) const {
    ...
    ValidatedReal vr1=_r1.compute_get(eff);
    ValidatedReal vr2=_r2.compute_get(eff);
...
template<> Int integer_cast<Int,Real>(Real const& r) { return std::round(r.compute(Accuracy(1)).get(dp).get_d()); }
template<> Nat integer_cast<Nat,Real>(Real const& r) { return std::round(r.compute(Accuracy(1)).get(dp).get_d()); }
...
FloatMPBounds Real::get(MultiplePrecision pr) const { return this->_ptr->_compute_get(Effort(pr.bits()/64),pr); }
...
ValidatedLowerReal::ValidatedLowerReal(DyadicLowerBound const& y) : _ptr(std::make_shared<ValidatedRealWrapper<DyadicBounds>>(y.raw())) { }
ValidatedLowerReal::ValidatedLowerReal(ValidatedReal const& r) : _ptr(r.managed_pointer()) { }
...
ValidatedUpperReal::ValidatedUpperReal(DyadicUpperBound const& y) : _ptr(std::make_shared<ValidatedRealWrapper<DyadicBounds>>(y.raw())) { }
ValidatedUpperReal::ValidatedUpperReal(ValidatedReal const& r) : _ptr(r.managed_pointer()) { }
